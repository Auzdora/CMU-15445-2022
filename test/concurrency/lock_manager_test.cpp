...
/** Upgrading single transaction test*/
void RowLockUpgradeTest1() {
  ...
  auto txn1 = txn_mgr.Begin();
  EXPECT_EQ(0, txn1->GetTransactionId());
  ...
  /** Take S lock on row */
  EXPECT_EQ(true, lock_mgr.LockRow(txn1, LockManager::LockMode::SHARED, oid, rid0));
  CheckTxnRowLockSize(txn1, oid, 1, 0);

  /** Upgrade from IS -> S */
  EXPECT_EQ(true, lock_mgr.LockTable(txn1, LockManager::LockMode::SHARED, oid));
  EXPECT_EQ(true, lock_mgr.UnlockRow(txn1, oid, rid0));
  CheckTableLockSizes(txn1, 1, 0, 0, 0, 0);
...
TEST(LockManagerTest, AbortRowTest1) {
  LockManager lock_mgr{};
  TransactionManager txn_mgr{&lock_mgr};
  table_oid_t oid = 0;
  ...
  std::thread t1([&]() {
    bool res;
    res = lock_mgr.LockTable(txn0, LockManager::LockMode::INTENTION_EXCLUSIVE, oid);
    EXPECT_TRUE(res);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    // lock_mgr.UnlockTable(txn0, oid);
    // txn_mgr.Commit(txn0);
