...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) {
    auto child_node = children_.find(key_char);
    if (child_node == children_.end()) {
      return nullptr;
    }
    return &children_[key_char];
  }
  ...
  void RemoveChildNode(char key_char) {
    auto v = children_.find(key_char);
    if (v == children_.end()) {
      return;
    }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    int keylen = static_cast<int>(key.length() - 1);

    latch_.WLock();
    auto *current = &root_;
    for (int i = 0; i < keylen; i++) {
      if (current->get()->HasChild(key[i])) {
        current = current->get()->GetChildNode(key[i]);
        continue;
      }
      current = current->get()->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
    }

    auto *endnode = current->get()->GetChildNode(key[keylen]);
    if (endnode == nullptr) {
      current->get()->InsertChildNode(key[keylen], std::make_unique<TrieNodeWithValue<T>>(key[keylen], value));
      latch_.WUnlock();
      return true;
    }
    if (!endnode->get()->IsEndNode()) {
      *endnode = std::make_unique<TrieNodeWithValue<T>>(std::move(*(endnode->get())), value);
      latch_.WUnlock();
      return true;
    }
    if (endnode->get()->IsEndNode()) {
  ...
  bool Remove(const std::string &key) {
    ...
    auto *current = &root_;
    for (char i : key) {
      if (!current->get()->HasChild(i)) {
        latch_.WUnlock();
        return false;
      }
      current = current->get()->GetChildNode(i);
    }

    auto *head = &root_;
    RecurRemove(std::move(*head), key, 0);
    latch_.WUnlock();
    return true;
  }

  bool RecurRemove(std::unique_ptr<TrieNode> &&node, const std::string &key, int depth) {
    ...
    if (node->HasChild(key[depth])) {
      if (RecurRemove(std::move(*node->GetChildNode(key[depth])), key, depth + 1)) {
        node->RemoveChildNode(key[depth]);
      }
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto endnode = dynamic_cast<TrieNodeWithValue<T> *>(current->get());
    if (endnode != nullptr) {
      *success = true;
      latch_.RUnlock();
